...
gen_y_h2 <- function(nsim,X,h2Y) {
  ...
  betamat = matrix(rt(msnp*nsim, df = 2),msnp, nsim)
  epsimat = matrix(rt(nsamp*nsim, df = 2),nsamp, nsim)
  epsimat = scale(epsimat) * sqrt(1 - h2Y)
  # sparse matrices
  #X = Matrix(X, sparse = TRUE)
  #betamat = Matrix(betamat, sparse = TRUE)
  # multiply and back to dense matrix
  gYmat = X %*% betamat
  #gYmat = mat.mult(X, betamat)
...
# run assoc
traits_nulls <- gen_null_chi2_fast(X,pred_traits,h,nsim)
